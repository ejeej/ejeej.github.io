...
:
  ...
  - Кривые Каплана-Майера оцифровывались с помощью EngaugeDigitizer, по ним восстанавливались IPD с помощью модифицированного алгоритма Гайот^[Каюсь, раньше называла его алгоритмом Гайота, но не так давно обратила внимание, что Гайот -- это женщина, Патрисия :)], реализованного в пакете IPDfromKM для R^[Liu N, Zhou Y, Lee JJ. IPDfromKM: reconstruct individual patient data from published Kaplan-Meier survival curves. BMC Med Res Methodol. 2021 Jun 1;21(1):111. doi: [10.1186/s12874-021-01308-8](https://doi.org/10.1186/s12874-021-01308-8)]. Кстати, если вы не владеете R, то разработчики пакета сделали [онлайн-приложение](https://biostatistics.mdanderson.org/shinyapps/IPDfromKM/), которое позволяет оцифровать кривую или восстановить IPD по координатам кривой, оцифрованной другими средствами.
  - При этом, если были опубликованы кривые выживаемости отдельно для подгрупп PD-L1-негативных и PD-L1-позитивных пациентов (с экспрессией PD-L1 < 1% или $\geq$ 1% по TPS или TPC), то IPD восстанавливались по ним, благодаря чему для этих исследований мы получали IPD по выживаемости + по ковариате для статуса эспрессии PD-L1 и могли их использовать для индивидуального уровня иерархической модели (см. ниже), наряду с исследованием, по которому у нас были оригинальные IPD.
...
:
  ...
  - Оценка модели с последующими расчетами выполнялась с помощью пакета `multinma`^[Phillippo, D. M. (2026). multinma: Bayesian Network Meta-Analysis of Individual and Aggregate Data. R package version 0.9.1, doi: 10.5281/zenodo.3904454]. У пакета есть замечательный [vignette](https://dmphillippo.github.io/multinma/), со ссылками на статьи, в которых разбираются особенности ML NMR, и примерами для разных конечных точек.
